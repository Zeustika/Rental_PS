...
if ($_GET['aksi'] == 'tambah') {
    ...
    if(!in_array($filetype, array_keys($allowedTypes))) {
        ...
        if (move_uploaded_file($tmp_name, $target_path)) {
            ...
            $sql = "INSERT INTO `playstation`(`type_sewa`, `jenis_ps`, `harga`, `deskripsi`, `status`, `gambar`) 
                VALUES (?,?,?,?,?,?)";
...
if ($_GET['aksi'] == 'edit') {
    ...
    $data[] = $id;
    $sql = "UPDATE playstation SET type_sewa= ?, jenis_ps=?, harga=?, deskripsi=?, status=?, gambar=?
        WHERE id_ps = ?";
    $row = $koneksi->prepare($sql);
    $row->execute($data);
